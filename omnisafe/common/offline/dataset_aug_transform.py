...
class OfflineDataset_AUG_Transform(Dataset):
    ...
    def _data_aug(self,data, savepath, env_name):
        ...
        for i in tqdm(range(len(data['obs']/1000))):
        ...
        # save data
        save_dir = os.path.join(savepath, f'{env_name}_aug_data.npz')
        if os.path.exists(save_dir):
            print(f'Warning: {save_dir} already exists.')
            print(f'Warning: {save_dir} will be overwritten.')
        np.savez(
            os.path.join(savepath, f'{env_name}_aug_data.npz'),
            obs=obs_aug,
            action=data['action'],
            reward=data['reward'],
            cost=data['cost'],
