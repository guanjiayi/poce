...
class OfflineDataset_AUG(Dataset):
    ...
    def __init__(  # pylint: disable=too-many-branches
        ...
        required_fields = {'obs', 'action', 'reward', 'cost', 'next_obs', 'done', 'cost_increase'}
        ...
        if self._pre_transfer:
            ...
            self.cost_increase = torch.from_numpy(data['cost_increase']).to(device=device)
        else:
            ...
            self.cost_increase = torch.Tensor(data['cost_increase'])
    ...
    def __getitem__(
        ...
        return (
            ...
            self.cost_increase[idx].to(device=self._device)
        )
    ...
    def sample(
        ...
        batch_cost_increase = self.cost_increase[indices]

        if self._pre_transfer:
            return (batch_obs, batch_action, batch_reward, batch_cost, batch_next_obs, batch_done, batch_cost_increase)
        ...
        return (
            ...
            batch_cost_increase.to(device=self._device)
        )

    def _data_aug(self,data):
        """ Supplementary the incremental cost. """
        M = 0.
        cost_d = np.zeros_like(data['reward'])
        obs_aug = np.zeros([data['obs'].shape[0], data['obs'].shape[1]+1])
        next_obs_aug = np.zeros([data['next_obs'].shape[0], data['next_obs'].shape[1]+1])
        first_step = True

        for i in range(len(data['obs'])):

            if first_step:
                cost_increase = data['cost'][i]
                M_next = data['cost'][i]
                first_step = False
            else:
                cost_increase = max(data['cost'][i]-M,0)
                M_next = M + cost_increase

            o_aug = np.append(data['obs'][i],M)
            next_o_aug = np.append(data['next_obs'][i], M_next)

            cost_d[i] = cost_increase
            obs_aug[i] = o_aug
            next_obs_aug[i] = next_o_aug
...
class OfflineDatasetWithInit(OfflineDataset_AUG):
