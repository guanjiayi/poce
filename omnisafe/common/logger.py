...
class Logger:  # pylint: disable=too-many-instance-attributes
    ...
    def __init__(  # pylint: disable=too-many-arguments,too-many-locals
        ...
        if self._use_wandb and self._maste_proc:  # pragma: no cover
            ...
            wandb.init(project=project, name=name, dir=self._log_dir, config=config, settings=wandb.Settings(program_relpath="run_gpt.py", disable_git=True, save_code=False))
            if config is not None:
