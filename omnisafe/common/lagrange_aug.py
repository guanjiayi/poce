...
class Lagrange_AUG:
    ...
    def __init__(
        ...
        self.cost_increase_limit: float = cost_increase_limit
        self.lambda_lr: float = lambda_lr
        self.lagrangian_upper_bound: float | None = lagrangian_upper_bound

        # lagrangian multiplier for the cost
        ...
        # lagrangian multiplier for the cost increase
        self.increase_lagrangian_multiplier: torch.nn.Parameter = torch.nn.Parameter(
            torch.as_tensor(init_value),
            requires_grad=True,
        )
        self.increase_lambda_optimizer: torch.optim.Optimizer = torch_opt(
            [
                self.increase_lagrangian_multiplier,
            ],
            lr=lambda_lr,
        )
    ...
    def compute_increase_lambda_loss(self, mean_ep_increase_cost: float) -> torch.Tensor:
        """Penalty loss for Lagrange multiplier.
        ...
        Returns:
            Penalty loss for Lagrange multiplier.
        """
        return -self.increase_lagrangian_multiplier * (mean_ep_increase_cost - self.cost_increase_limit)

    def update_increase_lagrange_multiplier(self, increase_Jc: float) -> None:
        r"""Update Lagrange multiplier (lambda).
        ...
        Args:
            Jc (float): mean episode cost.
        """
        self.increase_lambda_optimizer.zero_grad()
        increase_lambda_loss = self.compute_increase_lambda_loss(increase_Jc)
        increase_lambda_loss.backward()
        self.increase_lambda_optimizer.step()
        # self.increase_lagrangian_multiplier.data.clamp_(
        #     0.0,
        #     self.lagrangian_upper_bound,
        # )  # enforce: lambda in [0, inf]
