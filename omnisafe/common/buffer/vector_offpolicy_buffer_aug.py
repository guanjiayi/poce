...
class VectorOffPolicyBuffer_AUG(OffPolicyBuffer_AUG):
    ...
    def __init__(  # pylint: disable=super-init-not-called,too-many-arguments
        ...
        if isinstance(obs_space, Box):
            obs_buf = torch.zeros(
                (size, num_envs, int(*obs_space.shape)+1),
                dtype=torch.float32,
                device=device,
            )
        ...
        self.data = {
            ...
            # Cost increment
            'cost_increase': torch.zeros((size, num_envs), dtype=torch.float32, device=device),
        }
