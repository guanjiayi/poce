...
from omnisafe.models.actor import (
    VAE_AUG,
    GaussianLearningActor_AUG,
    GaussianSACActor_AUG,
    MLPActor_AUG,
    PerturbationActor_AUG,
)
from omnisafe.models.actor.actor_builder_aug import ActorBuilder_AUG
from omnisafe.models.base_aug import Critic_AUG
from omnisafe.models.critic.critic_builder_aug import CriticBuilder_AUG
from omnisafe.typing import OmnisafeSpace
from omnisafe.utils.config import ModelConfig


class ActorQCritic_CAUG(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        print('actor_q_critic_aug')

        self.actor: GaussianLearningActor_AUG | GaussianSACActor_AUG | MLPActor_AUG | VAE_AUG | PerturbationActor_AUG = ActorBuilder_AUG(
            obs_space=obs_space,
            act_space=act_space,
        ...
        self.reward_critic: Critic_AUG = CriticBuilder_AUG(
            obs_space=obs_space,
            act_space=act_space,
        ...
        self.target_reward_critic: Critic_AUG = deepcopy(self.reward_critic)
        for param in self.target_reward_critic.parameters():
            param.requires_grad = False
        self.target_actor: GaussianLearningActor_AUG | GaussianSACActor_AUG | MLPActor_AUG | VAE_AUG | PerturbationActor_AUG = deepcopy(
            self.actor,
        )
        ...
        if model_cfgs.critic.lr is not None:
            ...
            # )
            self.reward_critic_optimizer1: optim.Optimizer
            self.reward_critic_optimizer1 = optim.Adam(
                self.reward_critic.critic_0.parameters(),
                lr = model_cfgs.critic.lr,
            )
            self.reward_critic_optimizer2: optim.Optimizer
            self.reward_critic_optimizer2 = optim.Adam(
                self.reward_critic.critic_1.parameters(),
                lr = model_cfgs.critic.lr
            )
