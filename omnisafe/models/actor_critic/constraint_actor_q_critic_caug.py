...
from omnisafe.models.actor_critic.actor_q_critic_caug import ActorQCritic_CAUG
...
class ConstraintActorQCritic_CAUG(ActorQCritic_CAUG):
    ...
    def __init__(
        ...
        # Create the standard Q-value network for the cost
        self.cost_critic: Critic_AUG = CriticBuilder_AUG(
            obs_space=obs_space,
            act_space=act_space,
        ...
        self.target_cost_critic: Critic_AUG = deepcopy(self.cost_critic)
        ...
        # Create the increase Q-value network for the cost
        self.cost_increase_critic: Critic_AUG = CriticBuilder_AUG(
            obs_space =  obs_space,
            act_space = act_space,
            hidden_sizes = model_cfgs.critic.hidden_sizes,
            activation = model_cfgs.critic.activation,
        ...
        self.target_cost_increase_critic: Critic_AUG = deepcopy(self.cost_increase_critic)
        for param in self.target_cost_increase_critic.parameters():
            param.requires_grad = False
        self.add_module('cost_increase_critic', self.cost_increase_critic)
        if model_cfgs.critic.lr is not None:
            self.cost_increase_critic_optimizer: optim.Optimizer
            self.cost_increase_critic_optimizer = optim.Adam(
                self.cost_increase_critic.parameters(),
                lr = model_cfgs.critic.lr,
            )
