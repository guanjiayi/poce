...
class SafetyHalfCheetahVelocityEnv(HalfCheetahEnv):
    ...
    def step(self, action):
        ...
        # if x_velocity > self._velocity_threshold and x_velocity < 6:
        #     cost = float((x_velocity-self._velocity_threshold)/(6-self._velocity_threshold))
        # else:
        #     cost = float(x_velocity >= 6)

        if x_velocity <= self._velocity_threshold:
            cost = float(0)
        elif x_velocity < 7:
            cost = float((x_velocity-self._velocity_threshold)/(6-self._velocity_threshold))
        else:
            cost = float(x_velocity >= 7)
